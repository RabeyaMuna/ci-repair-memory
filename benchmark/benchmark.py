...
class CIFixBenchmark:
    ...
    def run_dataset(self, fix_repo_function, test_dataset=None):        
        ...
        with open(jobs_ids_file_path, "w") as writer:
            for datapoint in tqdm(test_dataset):
                ...
                if job_identificator is None:
                    continue
                self.jobs_ids.append(job_identificator)
                # Serialize fully before writing so interruption during JSON
                # encoding cannot leave a truncated JSONL record.
                writer.write(json.dumps(job_identificator) + "\n")
                writer.flush()
    ...
    def run_multilevel_evaluation(
        ...
        """
        Run multi-level evaluation on completed benchmark results.

        This performs three-level analysis:
        - Level 1: Targeted Fix (specific failure step from logs)
        - Level 2: Extended Fix (additional failures beyond first)
        - Level 3: Complete Fix (entire pipeline passes)

        Args:
            dataset_path: Path to dataset file (default: from config)
            results_file: Path to results JSONL (default: jobs_results_{model}.jsonl)
            output_file: Path to save detailed evaluation (default: multilevel_evaluation_{model}.jsonl)

        Returns:
            Summary statistics dictionary
        """
        from multilevel_evaluator import MultiLevelEvaluator

        # Initialize evaluator
        evaluator = MultiLevelEvaluator(self.config, self.credentials)

        # Set paths
        if dataset_path is None:
            dataset_path = getattr(self, "dataset_path", None)
            if dataset_path is None:
                dataset_path = os.path.join(self.config.base_dir, "dataset", "lca_dataset.parquet")

        if results_file is None:
            results_file = os.path.join(self.config.out_folder, f"jobs_results_{self.model_name}.jsonl")

        if output_file is None:
            output_file = os.path.join(self.config.out_folder, f"multilevel_evaluation_{self.model_name}.jsonl")

        print("\n" + "="*80)
        print("RUNNING MULTI-LEVEL EVALUATION")
        print("="*80)
        ...
        # Run evaluation
        summary = evaluator.evaluate_all_repairs(dataset_path, results_file, output_file)
